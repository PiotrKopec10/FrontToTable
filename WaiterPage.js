...
const WaiterPage = () => {
  ...
  const generateReport = () => {
    // Tutaj dodaj logikę generowania raportu
    console.log('Generowanie raportu...');
    // Możesz użyć danych z 'orders' do obliczeń i wygenerowania raportu
  ...
  return (
    <View style={{ flex: 1 }}>
      <ImageBackground source={require('./photo/BG1.png')} style={{ flex: 1, resizeMode: 'cover' }}>
        ...
        />
        <TouchableOpacity
          style={[WaiterPageStyles.raportButton, { margin: 15 }]}
          onPress={generateReport}
        >
          <Text style={WaiterPageStyles.addButtonText}>GENERUJ RAPORT</Text>
        </TouchableOpacity>


<Modal visible={isModalVisible} animationType="slide" transparent={true}>
  <View style={WaiterPageStyles.modalContainer}>

    <Text style={[WaiterPageStyles.header, {color: '#FFD983', fontSize: 40}]}>RAPORT{` restaurantId`}</Text>
    <Text style={[WaiterPageStyles.header, {color: '#FFD983', fontSize: 25}]}>{`Kelner ID: `}</Text>
    <Text style={[WaiterPageStyles.header, {color: 'white', fontSize: 25}]}>{`Ilość obsługiwanych stolików: `}</Text>
    <Text style={[WaiterPageStyles.header, {color: 'white', fontSize: 25}]}>{`Ilość przyjętych zamówień: `}</Text>
    <Text style={[WaiterPageStyles.header, {color: 'white', fontSize: 25}]}>{`Najczęstsza forma płatności: `}</Text>
    <Text style={[WaiterPageStyles.header, {color: 'white', fontSize: 25}]}>{`Kwota zarobiona przez obsłużone stoliki: `}</Text>


    <TouchableOpacity onPress={closeModal}>
      <Text style={WaiterPageStyles.closeModalButton}>Zamknij Raport</Text>
    </TouchableOpacity>
  </View>
</Modal>
